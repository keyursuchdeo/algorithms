...
object CreateSortedArrayThroughInstructions extends App {
  object Solution {
    def createSortedArray(instructions: Array[Int]): Int = {

      val modulo: Int = (Math.pow(10, 9).toInt + 7)

      def findMaxInsertPositionOf(num: Int, array: Array[Int]) = {
      ...
      def findMinInsertPositionOf(num: Int, array: Array[Int]) = {
        @scala.annotation.tailrec
        def find(low: Int, high: Int, pos: Int): Int = {
          if(high < low) {
            if (pos == -1) low else pos
      ...
      @scala.annotation.tailrec
      def findCost(index: Int, arrayTillNow: Array[Int], costTillNow: Int): Int = {
      ...
      findCost(1, Array(instructions.head), 0)
    }
  }
}
