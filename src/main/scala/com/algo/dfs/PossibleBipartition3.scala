...
object PossibleBipartition3 extends App {
  ...
  val n = 5
  val dislikes = Array(Array(1, 2), Array(3, 4), Array(4, 5), Array(3, 5))
    ...
    def possibleBipartition(N: Int, dislikes: Array[Array[Int]]): Boolean = {
      ...
      def find(index: Int): Boolean = {
        if (index == N + 1) {
          ...
          val output =
            verticesColour.get(index) match {
              case None => dfs(index, 0)
              case Some(currColour) => dfs(index, currColour)
            }
          if(output) find(index + 1) else output
        }
      }

      def dfs(vertex: Int, newColour: Int): Boolean = {
        verticesColour.get(vertex) match {
          case Some(currColour) => currColour == newColour
          case None =>
            verticesColour = verticesColour + (vertex -> newColour)
            println(verticesColour)

            for (neighbour <- adjList(vertex)) yield {
              dfs(neighbour, newColour ^ 1)
            }
            val dfsReturns: Seq[Boolean] = adjList(vertex).map(dfs(_, newColour ^ 1))
            !dfsReturns.contains(false)
        }
      }
